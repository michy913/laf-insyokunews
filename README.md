...
社員は下記URLをブックマークすれば、常に最新号を閲覧できる（毎週 月曜・水曜の自動更新でこのURLの内容が差し替わる）。

```
https://laftels-newsletter.netlify.app
```

- このURLは **閲覧専用モード**で表示される（`window.cowork` が無い環境では自動でそうなる）。
  AI生成ボタン・深掘り質問欄は非表示になり、スケジュールタスクが埋め込んだ最新号だけを表示する。
- 号データは各社員のブラウザの localStorage にも保存され、サイドバーから過去号を検索・閲覧できる。
...
- **スケジュール**: 毎週 月曜・水曜 9:00（cron: `0 9 * * 1,3`）
- **実行内容**:
  - STEP 1: 飲食業界ニュース**10件（大手5 + 中小5）** Exa検索
  - STEP 2: 人事労務トピック選定
  - STEP 3: アイスブレイク3件 + 労務提案ストーリー
  - STEP 4: 顧問先ニュース（現クラ71社を対象に、新店/閉店/M&A/取り組みを徹底検索）
